public class GameBoard {
	...
	public boolean checkUserWin(int r, int c) {
		...
		for(int i = -connect+1; i < connect; i++) {
			if(r+i >= 0 && r+i+1 < rows && c+i >= 0 && c+i+1 < columns) {
				if(board[r+i][c+i] == 1 && board[r+i+1][c+i+1] == 1) {
					count++;
				}
				else
					count = 0;
		...
		for(i = connect-1, k = -connect+1; i > -connect && k<connect; i--, k++) {
			if(r+i-1 >= 0 && r+i< rows && k+c+1 < columns && k+c >= 0) {
				if(board[r+i][c+k] == 1 && board[r+i-1][c+k+1] == 1) {
					count++;
				}
				else
					count = 0;
	...
	public boolean checkCompWin(int r, int c) {
		...
		for(int i = -connect+1; i < connect; i++) {
			if(r+i >= 0 && r+i+1 < rows && c+i >= 0 && c+i+1 < columns) {
				if(board[r+i][c+i] == 2 && board[r+i+1][c+i+1] == 2) {
					count++;
				}
				else
					count = 0;
		...
		for(i = connect-1, k = -connect+1; i > -connect && k<connect; i--, k++) {
			if(r+i-1 >= 0 && r+i< rows && k+c+1 < columns && k+c >= 0) {
				if(board[r+i][c+k] == 2 && board[r+i-1][c+k+1] == 2) {
					count++;
				}
				else
					count = 0;
